  ...
  function subscribeToMessages(conversationId) {
    ...
    realtimeRef.current = supabase
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages', filter: `conversation_id=eq.${conversationId}` }, async payload => {
        const { data } = await supabase.from('messages').select('id, content, type, created_at, sender_id, message_attachments (id, object_key, file_name, mime_type, size_bytes)').eq('id', payload.new.id).single()
        if (data) {
          setMessages(prev => {
