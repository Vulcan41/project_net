      ...
      {icons.map(icon => {
        ...
        return (
          <button key={icon.id} onClick={() => onSelect(icon.id)} title={icon.label}
            style={{ width: '34px', height: '34px', borderRadius: isActive ? '12px' : '50%', background: isActive ? 'var(--accent)' : 'transparent', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'border-radius 0.2s, background 0.2s' }}
